...
class SetupForm
{
    public function render(array $parameter, TypoScriptConstantsViewHelper $viewHelper)
    {
        ...
        return "<input name=\"$parameter[fieldName]\" value=\"$parameter[fieldValue]\" id=\"em-$parameter[propertyName]\"/ size=\"45\">
            <button onclick=\"edusharingSetup(event)\">Setup repository</button>
            <script>
                function edusharingSetup(event) {
                    event.preventDefault();
                    const repoUrl = $('#em-$parameter[propertyName]')[0].value;
                    if (!confirm(
                    ...
                    $.get(url)
                        .then(() => {
                            ...
                            $('[data-identifier=actions-view-go-back]')[0].click();
                        })
                        .catch((e) => {

        ...
        editor.addCommand('openModal', {
            exec: function (editor) {
                require([
                    ...
                    var selection = editor.getSelection().getSelectedElement() || editor.getSelection().getStartElement();
                            ...
                            parent.window.addEventListener("message", function (event) {
                                if (event.data.event == "APPLY_NODE") {
                                    ...
                                    if (event.data.data.properties['ccm:height']) {
                                        node.height = event.data.data.properties['ccm:height'][0];
                                    }
                                    ...
                                    if (event.data.data.properties['cclom:version']) {
                                        node.version = event.data.data.properties['cclom:version'][0];
                                    }
...
function bindAction(editor) {
    ...
    $(parent.document).on('click', '#edusharing_savedsearch_repick_a', function () {
        parent.document.getElementById('edusharing_repository_frame').src = appconfig.repo_url + 'components/search?ticket=' + ticket + '&reurl=IFRAME' + '&savedQuery=' + node.id;
...
function setNode(selection) {
    ...
    node.savedsearch_sortproperty = selection.getAttribute('data-edusharing_savedsearch_sortproperty');
...
function applyValues(editor) {
    ...
    if (node.mediatype == 'saved_search') {
        ...
        node.savedsearch_limit = parent.document.getElementById('edusharing_savedsearch_limit').value;
        node.savedsearch_sortproperty = parent.document.getElementById('edusharing_savedsearch_order').value;
        node.savedsearch_template = parent.document.getElementById('edusharing_savedsearch_template').value;
        setSavedSearchPreview(editor);
    } else {
...
function setSavedSearchPreview(editor) {
    $.ajax({
        ...
        success: function (response) {
            if (parent) {
                parent.document.getElementById('edusharing_savedsearch_preview').innerHTML = JSON.parse(response);
            } else {
                document.getElementById('edusharing_savedsearch_preview').innerHTML = JSON.parse(response);
            }
        },
        error: function () {
            if (parent) {
                ...
                document.getElementById('edusharing_savedsearch_preview').innerHTML = editor.lang.edusharing.saved_search_preview_error;
            }
        }
    });
}
...
function getPreviewUrl() {
    return appconfig.repo_url + 'preview?nodeId=' + node.id + '&ticket=' + ticket;
}
...
function insertEduObject(editor) {
    ...
    if (parent.document.querySelector('input[name="edusharing_version"]:checked'))
        obj.setAttribute('data-edusharing_version', parent.document.querySelector('input[name="edusharing_version"]:checked').value);
    else
        obj.setAttribute('data-edusharing_version', node.version);
...
function getEditDialogHtml(editor) {
    if (node.mimetype.indexOf('video') >= 0 || node.mimetype.indexOf('image') >= 0) {
        var html = '<div id="edusharing_object_dialog">' +
            '<img id="edusharing_preview_image" src="' + node.src + '">' +
            '<label for="edusharing_title_display" id="edusharing_title_display_label">' + editor.lang.edusharing.title + '</label>' +
            '<h1 id="edusharing_title_display">' + node.title + '</h1>' +
            '<fieldset id="edusharing_fieldset_dimensions">' +
            '<h2>' + editor.lang.edusharing.dimensions + '</h2>' +
            '<div>' +
            '<label class="edusharing_label">' + editor.lang.edusharing.height + '</label><br/>' +
            '<input value="' + node.height + '" id="edusharing_height" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="4" size="4"> px' +
            '</div>' +
            '<div>' +
            '<label class="edusharing_label">' + editor.lang.edusharing.width + '</label><br/>' +
            '<input value="' + node.width + '" id="edusharing_width" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="4" size="4"> px' +
            '</div>' +
            '</fieldset>';

        html += '<fieldset id="edusharing_fieldset_float">' +
            '<h2>' + editor.lang.edusharing.float + '</h2>' +
            '<input type="radio" id="edusharing_float_left" name="edusharing_float" value="left" ' + (node.float == 'left' ? 'checked' : '') + '>' +
            '<label for="edusharing_float_left">' + editor.lang.edusharing.float_left + '</label><br/>' +
            '<input type="radio" id="edusharing_float_right" name="edusharing_float" value="right" ' + (node.float == 'right' ? 'checked' : '') + '>' +
            '<label for="edusharing_float_right">' + editor.lang.edusharing.float_right + '</label><br/>' +
            '<input type="radio" id="edusharing_float_none" name="edusharing_float" value="none" ' + ((node.float != 'left' && node.float != 'right') ? 'checked' : '') + '>' +
            '<label for="edusharing_float_none">' + editor.lang.edusharing.float_none + '</label>' +
            '</fieldset>';
    } else if (node.mediatype != 'saved_search') {
        var html = '<div id="edusharing_object_dialog">' +
            '<img id="edusharing_preview_image" src="' + node.src + '">' +
    ...
    if (node.mediatype == 'saved_search') {
        var html = '<iframe style="display: none" id="edusharing_repository_frame" src=""></iframe>' +
            '<div id="edusharing_object_dialog">' +
            '<label for="edusharing_title_display" id="edusharing_title_display_label">' + editor.lang.edusharing.title + '</label>' +
            '<h1 id="edusharing_title_display">' + node.title + '</h1>' +
            '<fieldset id="edusharing_fieldset_savedsearch">' +
            '<div id="edusharing_savedsearch_preview"></div>' +
            '<div id="edusharing_savedsearch_repick"><a id="edusharing_savedsearch_repick_a" href="#">' + editor.lang.edusharing.savedsearch_edit + '</a></div>' +
            '<div id="edusharing_savedsearch_settings">' +
            '<h2>' + editor.lang.edusharing.advancedsettings + '</h2>' +
            '<label for="edusharing_savedsearch_limit">' + editor.lang.edusharing.savedsearch_limit_a + '</label>' +
            '<select id="edusharing_savedsearch_limit" name="edusharing_savedsearch_limit">' +
            '<option value="5" ' + (node.savedsearch_limit == '5' ? 'selected="selected"' : '') + '>5</option><option value="10" ' + (node.savedsearch_limit == '10' ? 'selected="selected"' : '') + '>10</option><option value="20" ' + (node.savedsearch_limit == '20' ? 'selected="selected"' : '') + '>20</option><option value="10000" ' + (node.savedsearch_limit == '10000' ? 'selected="selected"' : '') + '>' + editor.lang.edusharing.savedsearch_limit_all + '</option>' +
            '</select> ' + editor.lang.edusharing.savedsearch_limit_b +
            '<label for="edusharing_savedsearch_order">' + editor.lang.edusharing.savedsearch_order_a + '</label>' +
            '<select id="edusharing_savedsearch_order" name="edusharing_savedsearch_order"><option value="cm:modified">' + editor.lang.edusharing.savedsearch_order_modified + '</option></select> ' + editor.lang.edusharing.savedsearch_order_c +
            '<label for="edusharing_savedsearch_template">' + editor.lang.edusharing.savedsearch_template + ':</label>' +
            '<select id="edusharing_savedsearch_template" name="edusharing_savedsearch_template"><option value="card" ' + (node.savedsearch_template == 'card' ? 'selected="selected"' : '') + '>' + editor.lang.edusharing.savedsearch_template_card + '</option><option value="list" ' + (node.savedsearch_template == 'list' ? 'selected="selected"' : '') + '>' + editor.lang.edusharing.savedsearch_template_list + '</option></select>' +
            '</div>' +
            '</fieldset>';
    } else {
        html += '<fieldset id="edusharing_fieldset_version" style="display: none">' +
            '<h2>' + editor.lang.edusharing.version + '</h2>' +
            '<span id="edusharing_version_display_text">' + editor.lang.edusharing.version_display + ': ' + '<span id="edusharing_version_display"></span></span><br/>' +
            '<input type="radio" id="edusharing_version_current" name="edusharing_version" value="' + (node.version != '-1' ? node.version : '') + '" ' + (node.version != '-1' ? 'checked' : '') + '>' +
            '<label for="edusharing_version_current">' + editor.lang.edusharing.version_current + '</label><br/>' +
            '<input type="radio" id="edusharing_version_latest" name="edusharing_version" value="-1" ' + (node.version == '-1' ? 'checked' : '') + '>' +
            '<label for="edusharing_version_latest">' + editor.lang.edusharing.version_latest + '</label>' +
            '</fieldset>';
    }
    html += '</div>';
    return html;
...
function getInsertDialogHtml(editor) {
    var html = '<iframe id="edusharing_repository_frame" src="' + appconfig.repo_url + 'components/search?ticket=' + ticket + '&reurl=IFRAME"></iframe>' +
        ...
        '<fieldset id="edusharing_fieldset_savedsearch">' +
        '<div id="edusharing_savedsearch_preview"></div>' +
        '<div id="edusharing_savedsearch_repick"><a href="#">' + editor.lang.edusharing.savedsearch_edit + '</a></div>' +
        '<div id="edusharing_savedsearch_settings">' +
        '<h2>' + editor.lang.edusharing.advancedsettings + '</h2>' +
        '<label for="edusharing_savedsearch_limit">' + editor.lang.edusharing.savedsearch_limit_a + '</label>' +
        '<select id="edusharing_savedsearch_limit" name="edusharing_savedsearch_limit">' +
        '<option value="5">5</option><option value="10">10</option><option value="20">20</option><option value="10000">' + editor.lang.edusharing.savedsearch_limit_all + '</option>' +
        '</select> ' + editor.lang.edusharing.savedsearch_limit_b +
        '<label for="edusharing_savedsearch_order">' + editor.lang.edusharing.savedsearch_order_a + '</label>' +
        '<select id="edusharing_savedsearch_order" name="edusharing_savedsearch_order"><option value="cm:modified">' + editor.lang.edusharing.savedsearch_order_modified + '</option></select> ' + editor.lang.edusharing.savedsearch_order_c +
        '<label for="edusharing_savedsearch_template">' + editor.lang.edusharing.savedsearch_template + ':</label>' +
        '<select id="edusharing_savedsearch_template" name="edusharing_savedsearch_template"><option value="card">' + editor.lang.edusharing.savedsearch_template_card + '</option><option value="list">' + editor.lang.edusharing.savedsearch_template_list + '</option></select>' +
        '</div>' +
        '</fieldset>' +
        '</div>';
